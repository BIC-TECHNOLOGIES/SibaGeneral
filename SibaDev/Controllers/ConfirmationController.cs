...
namespace SibaDev.Controllers
{
    public class ConfirmationController : ApiController
    {
        ...
        [HttpGet] 
        [Route("api/Confirmation/confirmLiabilityPolicy/{polId}")]       
        public object ConfirmLiability(int polId)
        {
            var db = new SibaModel();
            try
            {
                var polHead = PolicyLiabilityMdl.get_policy(polId);
                ...
                if (premRegs.Any())
                {
                    ...
                    if (acntInts.Any())
                    {
                        ...
                        var polFees = (from f in db.INS_UDW_POL_FEES where f.POL_FEE_POL_SYS_ID == polId select f).ToList();
                        var commissions = (from c in db.INS_UWD_INTERMEDIARY_COMM where c.CMM_POL_SYS_ID == polId select c).ToList();
                        var prodrisk = (from u in db.INS_UDW_LIABILITY where u.LIA_POLH_SYS_ID == polId select u).ToList();
                        var wkscomp = (from u in db.INS_UDW_LIABILITY_WKS_COMP where u.LIAWOC_POLH_SYS_ID == polId select u).ToList();
                        var acntMod = new PolicyLiabilityApprovalMdl();
                        var result = acntMod.ForwardTransaction(polHead, prodrisk, wkscomp, premReg, acntInt, polFees, commissions);
                        return result;
                    }
