...
{
    public class PolicyTransitApprovalMdl : Model
    {
        // Policy Confirmations
        
        public object ForwardTransaction(INS_UWD_POLICY_HEAD polHead, List<INS_UDW_GA_TRANSIT> prodrisk, INS_PREMIUM_REGISTER premRegister, MS_ACCTS_INTEGRATION acntInt, List<INS_UDW_POL_FEES> polFees, List<INS_UWD_INTERMEDIARY_COMM> commissions)
        {
                        ...
                        prodrisk.ForEach(risk =>
                        {
                            if (risk != null)
                            {
                                ...
                                if (grpDtls != null)
                                {
                                    ...
                                    var tttyMsHead = treatyGrpHead.MS_RI_PTTY_HEAD.FirstOrDefault(t => t.TH_CURRENCY == polHead.POLH_CURRENCY && risk.TRANS_UW_YEAR == t.TH_UWYR);
                                    if (tttyMsHead != null)
                                    {

                                        var ttyhead = new INS_RI_TTY_HEAD
                                        {
                                            TTH_ACC_SI_BC = risk.TRANS_RI_SI_BC,
                                            TTH_ACC_SI_FC = risk.TRANS_RI_SI_FC,
                                            TTH_ADJ_YN = null,
                                            ...
                                            TTH_FROM_DATE = risk.TRANS_START_DATE,
                                            ...
                                            TTH_RISK_SYS_ID = risk.TRANS_SYS_ID,
                                            TTH_RI_ACC_YN = null,
                                            TTH_RI_PREM_BC = risk.TRANS_RI_PREM_BC,
                                            TTH_RI_PREM_FC = risk.TRANS_RI_PREM_FC,
                                            TTH_RI_SI_BC = risk.TRANS_RI_SI_BC,
                                            TTH_RI_SI_FC = risk.TRANS_RI_SI_FC,
                                            TTH_SC_CODE = polHead.POLH_SUB_CLASS_CODE,
                                            TTH_STATUS = "A",
                                            TTH_TOTAL_PREM_BC = risk.TRANS_TOT_PREM_BC,
                                            TTH_TOTAL_PREM_FC = risk.TRANS_TOT_PREM_FC,
                                            TTH_TOTAL_SI_BC = risk.TRANS_SI_BC,
                                            TTH_TOTAL_SI_FC = risk.TRANS_SI_FC,
                                            TTH_TO_DATE = risk.TRANS_END_DATE,
                                            TTH_UW_YEAR = (short?)risk.TRANS_UW_YEAR
                                        };
                                        db.INS_RI_TTY_HEAD.Add(ttyhead);
                                        db.SaveChanges();

                                        var excessSIFC = risk.TRANS_SI_FC;
                                        var excessSIBC = risk.TRANS_SI_BC;
                                        ...
                                        var exxAll = new INS_RI_TTY_ALLOCATION
                                        {
                                            ...
                                            TTA_TTY_LIMIT_FC = 0,
                                            TTA_TTY_LIMIT_BC = 0,
                                            TTA_UW_YEAR = (short?)risk.TRANS_UW_YEAR
                                        };
                                        ...
                                        foreach (var detail in tttyMsHead.MS_RI_PTTY_DETL.OrderBy(d => d.TD_SEQ))
                                        {
                                            ...
                                            allocation.TTA_ALLOC_PERC = (allocation.TTA_TTY_SI_FC / risk.TRANS_SI_FC) * 100;
                                            allocation.TTA_TTY_PREM_BC = risk.TRANS_TOT_PREM_BC * (allocation.TTA_ALLOC_PERC / 100);
                                            allocation.TTA_TTY_PREM_FC = risk.TRANS_TOT_PREM_FC * (allocation.TTA_ALLOC_PERC / 100);

                                            //Update FAC Excess record
                                            exxAll.TTA_TTY_SI_FC = excessSIFC;
                                            exxAll.TTA_TTY_SI_BC = excessSIBC;
                                            exxAll.TTA_ALLOC_PERC = (excessSIFC / risk.TRANS_SI_FC) * 100;
                                            exxAll.TTA_TTY_PREM_BC = risk.TRANS_TOT_PREM_BC * (exxAll.TTA_ALLOC_PERC / 100);
                                            exxAll.TTA_TTY_PREM_FC = risk.TRANS_TOT_PREM_FC * (exxAll.TTA_ALLOC_PERC / 100);
        ...
        public object InsuranceTransactionPosting(IEnumerable<INSURANCE_LEDGER> ledger)
        {
            ...
            using (var trans = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    policy.INS_UDW_GA_TRANSIT.ForEach(b =>
                    {
                        var historyrisk = new INS_UDW_HGA_TRANSIT();
                        historyrisk.Map(b);
                        db.INS_UDW_HGA_TRANSIT.Add(historyrisk); 
