...
{
    public class PolicyPaApprovalMdl : Model
    {
        // Policy Confirmations
        
        public object ForwardTransaction(INS_UWD_POLICY_HEAD polHead, List<INS_UDW_PERSONAL_ACCIDENT> prodrisk, INS_PREMIUM_REGISTER premRegister, MS_ACCTS_INTEGRATION acntInt, List<INS_UDW_POL_FEES> polFees, List<INS_UWD_INTERMEDIARY_COMM> commissions)
        {
                        ...
                        prodrisk.ForEach(risk =>
                        {
                            if (risk != null)
                            {
                                ...
                                if (grpDtls != null)
                                {
                                    ...
                                    var tttyMsHead = treatyGrpHead.MS_RI_PTTY_HEAD.FirstOrDefault(t => t.TH_CURRENCY == polHead.POLH_CURRENCY && risk.PA_UW_YEAR == t.TH_UWYR);
                                    if (tttyMsHead != null)
                                    {

                                        var ttyhead = new INS_RI_TTY_HEAD
                                        {
                                            TTH_ACC_SI_BC = risk.PA_RI_SI_BC,
                                            TTH_ACC_SI_FC = risk.PA_RI_SI_FC,
                                            TTH_ADJ_YN = null,
                                            ...
                                            TTH_FROM_DATE = risk.PA_START_DATE,
                                            ...
                                            TTH_RISK_SYS_ID = risk.PA_SYS_ID,
                                            TTH_RI_ACC_YN = null,
                                            TTH_RI_PREM_BC = risk.PA_RI_PREM_BC,
                                            TTH_RI_PREM_FC = risk.PA_RI_PREM_FC,
                                            TTH_RI_SI_BC = risk.PA_RI_SI_BC,
                                            TTH_RI_SI_FC = risk.PA_RI_SI_FC,
                                            TTH_SC_CODE = polHead.POLH_SUB_CLASS_CODE,
                                            TTH_STATUS = "A",
                                            TTH_TOTAL_PREM_BC = risk.PA_TOT_PREM_BC,
                                            TTH_TOTAL_PREM_FC = risk.PA_TOT_PREM_FC,
                                            TTH_TOTAL_SI_BC = risk.PA_SI_BC,
                                            TTH_TOTAL_SI_FC = risk.PA_SI_FC,
                                            TTH_TO_DATE = risk.PA_END_DATE,
                                            TTH_UW_YEAR = (short?)risk.PA_UW_YEAR
                                        };
                                        db.INS_RI_TTY_HEAD.Add(ttyhead);
                                        db.SaveChanges();

                                        var excessSIFC = risk.PA_SI_BC;
                                        var excessSIBC = risk.PA_SI_FC;
                                        ...
                                        var exxAll = new INS_RI_TTY_ALLOCATION
                                        {
                                            ...
                                            TTA_TTY_LIMIT_FC = 0,
                                            TTA_TTY_LIMIT_BC = 0,
                                            TTA_UW_YEAR = (short?)risk.PA_UW_YEAR
                                        };
                                        ...
                                        foreach (var detail in tttyMsHead.MS_RI_PTTY_DETL.OrderBy(d => d.TD_SEQ))
                                        {
                                            ...
                                            allocation.TTA_ALLOC_PERC = (allocation.TTA_TTY_SI_FC / risk.PA_SI_FC) * 100;
                                            allocation.TTA_TTY_PREM_BC = risk.PA_TOT_PREM_BC * (allocation.TTA_ALLOC_PERC / 100);
                                            allocation.TTA_TTY_PREM_FC = risk.PA_TOT_PREM_FC * (allocation.TTA_ALLOC_PERC / 100);

                                            //Update FAC Excess record
                                            exxAll.TTA_TTY_SI_FC = excessSIFC;
                                            exxAll.TTA_TTY_SI_BC = excessSIBC;
                                            exxAll.TTA_ALLOC_PERC = (excessSIFC / risk.PA_SI_FC) * 100;
                                            exxAll.TTA_TTY_PREM_BC = risk.PA_TOT_PREM_BC * (exxAll.TTA_ALLOC_PERC / 100);
                                            exxAll.TTA_TTY_PREM_FC = risk.PA_TOT_PREM_FC * (exxAll.TTA_ALLOC_PERC / 100);
