...
{
    public class PolicyLiabilityApprovalMdl : Model
    {
        // Policy Confirmations
        
        public object ForwardTransaction(INS_UWD_POLICY_HEAD polHead, List<INS_UDW_LIABILITY> prodrisk, List<INS_UDW_LIABILITY_WKS_COMP> wkscomp, INS_PREMIUM_REGISTER premRegister, MS_ACCTS_INTEGRATION acntInt, List<INS_UDW_POL_FEES> polFees, List<INS_UWD_INTERMEDIARY_COMM> commissions)
        {
                    ...
                    if (premRegister.PR_ACCT_YN == "Y" && accountTotalBc > 0 && accountTotalFc > 0)
                    {
                        ...
                        prodrisk.ForEach(risk =>
                        {
                            if (risk != null)
                            {
                                ...
                                if (grpDtls != null)
                                {
                                    ...
                                    var tttyMsHead = treatyGrpHead.MS_RI_PTTY_HEAD.FirstOrDefault(t => t.TH_CURRENCY == polHead.POLH_CURRENCY && risk.LIA_UW_YEAR == t.TH_UWYR);
                                    if (tttyMsHead != null)
                                    {

                                        var ttyhead = new INS_RI_TTY_HEAD
                                        {
                                            TTH_ACC_SI_BC = risk.LIA_RI_SI_BC,
                                            TTH_ACC_SI_FC = risk.LIA_RI_SI_FC,
                                            TTH_ADJ_YN = null,
                                            ...
                                            TTH_FROM_DATE = risk.LIA_START_DATE,
                                            ...
                                            TTH_RISK_SYS_ID = risk.LIA_SYS_ID,
                                            TTH_RI_ACC_YN = null,
                                            TTH_RI_PREM_BC = risk.LIA_RISK_PREM_BC,
                                            TTH_RI_PREM_FC = risk.LIA_RISK_PREM_FC,
                                            TTH_RI_SI_BC = risk.LIA_RI_SI_BC,
                                            TTH_RI_SI_FC = risk.LIA_RI_SI_FC,
                                            TTH_SC_CODE = polHead.POLH_SUB_CLASS_CODE,
                                            TTH_STATUS = "A",
                                            TTH_TOTAL_PREM_BC = risk.LIA_TOT_PREM_BC,
                                            TTH_TOTAL_PREM_FC = risk.LIA_TOT_PREM_FC,
                                            TTH_TOTAL_SI_BC = risk.LIA_SI_BC,
                                            TTH_TOTAL_SI_FC = risk.LIA_SI_FC,
                                            TTH_TO_DATE = risk.LIA_END_DATE,
                                            TTH_UW_YEAR = (short?)risk.LIA_UW_YEAR
                                        };
                                        db.INS_RI_TTY_HEAD.Add(ttyhead);
                                        db.SaveChanges();

                                        var excessSIFC = risk.LIA_SI_FC;
                                        var excessSIBC = risk.LIA_SI_BC;
                                        ...
                                        var exxAll = new INS_RI_TTY_ALLOCATION
                                        {
                                            ...
                                            TTA_TTY_LIMIT_FC = 0,
                                            TTA_TTY_LIMIT_BC = 0,
                                            TTA_UW_YEAR = (short?)risk.LIA_UW_YEAR
                                        };
                                        ...
                                        foreach (var detail in tttyMsHead.MS_RI_PTTY_DETL.OrderBy(d => d.TD_SEQ))
                                        {
                                            ...
                                            allocation.TTA_ALLOC_PERC = (allocation.TTA_TTY_SI_FC / risk.LIA_SI_FC) * 100;
                                            allocation.TTA_TTY_PREM_BC = risk.LIA_TOT_PREM_BC * (allocation.TTA_ALLOC_PERC / 100);
                                            allocation.TTA_TTY_PREM_FC = risk.LIA_TOT_PREM_FC * (allocation.TTA_ALLOC_PERC / 100);

                                            //Update FAC Excess record
                                            exxAll.TTA_TTY_SI_FC = excessSIFC;
                                            exxAll.TTA_TTY_SI_BC = excessSIBC;
                                            exxAll.TTA_ALLOC_PERC = (excessSIFC / risk.LIA_SI_FC) * 100;
                                            exxAll.TTA_TTY_PREM_BC = risk.LIA_TOT_PREM_BC * (exxAll.TTA_ALLOC_PERC / 100);
                                            exxAll.TTA_TTY_PREM_FC = risk.LIA_TOT_PREM_FC * (exxAll.TTA_ALLOC_PERC / 100);
                        ...
                        wkscomp.ForEach(risk =>
                        {
                            if (risk != null)
                            {
                                ...
                                {
                                    var treatyGrpHead = grpDtls.MS_RI_GROUP_HEAD;
                                    var tttyMsHead = treatyGrpHead.MS_RI_PTTY_HEAD.FirstOrDefault(t => t.TH_CURRENCY == polHead.POLH_CURRENCY && risk.LIAWOC_UW_YEAR == t.TH_UWYR);
                                    if (tttyMsHead != null)
                                    {

                                        var ttyhead = new INS_RI_TTY_HEAD
                                        {
                                            TTH_ACC_SI_BC = risk.LIAWOC_RI_SI_BC,
                                            TTH_ACC_SI_FC = risk.LIAWOC_RI_SI_FC,
                                            TTH_ADJ_YN = null,
                                            TTH_CLASS_CODE = polHead.POLH_CLASS_CODE,
                                            TTH_COMB_LOC_ID = null,
                                            TTH_CRTE_BY = "System",
                                            TTH_CRTE_DATE = DateTime.Now,
                                            TTH_EML_PERC = grpDtls.GD_PML_PERC,
                                            TTH_FROM_DATE = risk.LIAWOC_START_DATE,
                                            TTH_INS_SRCE = polHead.POLH_INS_SOURCE,
                                            TTH_LOC_END_NO = (int?)polHead.POLH_END_NO,
                                            TTH_LOC_SYS_ID = null,
                                            TTH_MOD_BY = null,
                                            ...
                                            //TTH_REF_ID = risk.BOND_SYS_ID,
                                            TTH_RISK_END_NO = (int?)polHead.POLH_END_NO,
                                            TTH_RISK_SYS_ID = risk.LIAWOC_SYS_ID,
                                            TTH_RI_ACC_YN = null,
                                            TTH_RI_PREM_BC = risk.LIAWOC_RISK_PREM_BC,
                                            TTH_RI_PREM_FC = risk.LIAWOC_RISK_PREM_FC,
                                            TTH_RI_SI_BC = risk.LIAWOC_RI_SI_BC,
                                            TTH_RI_SI_FC = risk.LIAWOC_RI_SI_FC,
                                            TTH_SC_CODE = polHead.POLH_SUB_CLASS_CODE,
                                            TTH_STATUS = "A",
                                            TTH_TOTAL_PREM_BC = risk.LIAWOC_TOT_PREM_BC,
                                            TTH_TOTAL_PREM_FC = risk.LIAWOC_TOT_PREM_FC,
                                            TTH_TOTAL_SI_BC = risk.LIAWOC_SI_BC,
                                            TTH_TOTAL_SI_FC = risk.LIAWOC_SI_FC,
                                            TTH_TO_DATE = risk.LIAWOC_END_DATE,
                                            TTH_UW_YEAR = (short?)risk.LIAWOC_UW_YEAR
                                        };
                                        db.INS_RI_TTY_HEAD.Add(ttyhead);
                                        db.SaveChanges();

                                        var excessSIFC = risk.LIAWOC_SI_FC;
                                        var excessSIBC = risk.LIAWOC_SI_BC;
                                        ...
                                        var exxAll = new INS_RI_TTY_ALLOCATION
                                        {
                                            ...
                                            TTA_TTY_LIMIT_FC = 0,
                                            TTA_TTY_LIMIT_BC = 0,
                                            TTA_UW_YEAR = (short?)risk.LIAWOC_UW_YEAR
                                        };
                                        ...
                                        foreach (var detail in tttyMsHead.MS_RI_PTTY_DETL.OrderBy(d => d.TD_SEQ))
                                        {
                                            ...
                                            allocation.TTA_ALLOC_PERC = (allocation.TTA_TTY_SI_FC / risk.LIAWOC_SI_FC) * 100;
                                            allocation.TTA_TTY_PREM_BC = risk.LIAWOC_TOT_PREM_BC * (allocation.TTA_ALLOC_PERC / 100);
                                            allocation.TTA_TTY_PREM_FC = risk.LIAWOC_TOT_PREM_FC * (allocation.TTA_ALLOC_PERC / 100);

                                            //Update FAC Excess record
                                            exxAll.TTA_TTY_SI_FC = excessSIFC;
                                            exxAll.TTA_TTY_SI_BC = excessSIBC;
                                            exxAll.TTA_ALLOC_PERC = (excessSIFC / risk.LIAWOC_SI_FC) * 100;
                                            exxAll.TTA_TTY_PREM_BC = risk.LIAWOC_TOT_PREM_BC * (exxAll.TTA_ALLOC_PERC / 100);
                                            exxAll.TTA_TTY_PREM_FC = risk.LIAWOC_TOT_PREM_FC * (exxAll.TTA_ALLOC_PERC / 100);
                                            exxAll.TTA_SEQ_NO = (byte)(detail.TD_SEQ + 1);
                                            //add allocations
                                            db.INS_RI_TTY_ALLOCATION.Add(allocation);
        ...
        public object InsuranceTransactionPosting(IEnumerable<INSURANCE_LEDGER> ledger)
        {
            ...
            using (var trans = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    policy.INS_UDW_LIABILITY.ForEach(b =>
                    {
                        var historyrisk = new INS_UDW_HLIABILITY();
                        historyrisk.Map(b);
                        db.INS_UDW_HLIABILITY.Add(historyrisk);

                        b.INS_UDW_LIABILITY_FEES.ForEach(c =>
                        {
                            var historyfees = new INS_UDW_HLIABILITY_FEES();
                            historyfees.Map(c);
                            db.INS_UDW_HLIABILITY_FEES.Add(historyfees);
                        });
                    ...
                    policy.INS_UDW_LIABILITY_WKS_COMP.ForEach(b =>
                    {
                        var historyrisk = new INS_UDW_HLIABILITY_WKS_COMP();
                        historyrisk.Map(b);
                        db.INS_UDW_HLIABILITY_WKS_COMP.Add(historyrisk);
