    ...
    public class PolicyBondMdl : Model
    {
        ...
        public static INS_UWD_POLICY_HEAD SaveEndsmntCancl(INS_UWD_POLICY_HEAD viewPolh)
        {
            ...
            if (dbPolh != null)
            {
                 ...
                 * risk update
                 *--------------*/
                foreach (var risk in viewPolh.INS_UDW_BONDS)
                {
                    var dbrisk = db.INS_UDW_BONDS.Find(risk.BOND_SYS_ID);
                    switch (risk.BOND_STATUS)
                    {
                        case "A":
                            if (dbrisk != null)
                            {
                                db.INS_UDW_BONDS.Attach(dbrisk);
                                dbrisk.Map(risk);
                            }
                        ...
                        case "U":
                            risk.BOND_STATUS = "A";
                            risk.BOND_POLH_SYS_ID = viewPolh.POLH_SYS_ID;
                            db.INS_UDW_BONDS.Add(risk);

                            break;
                        case "D":
                            db.INS_UDW_BONDS.Remove(db.INS_UDW_BONDS.Find(risk.BOND_SYS_ID));
                            break;
                    }
