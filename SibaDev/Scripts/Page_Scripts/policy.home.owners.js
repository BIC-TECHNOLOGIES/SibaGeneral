        ...
        $scope.homeowners_grid = u.default_grid("#gridHomeOwnersRisks", "#gridHomeOwnersRisksPager", "Home Owners Policy",
            ...
            function (sel_id) {
                ...
                u.fill_form({
                    ...
                    HOP_SUM_INSURED: grid.jqGrid('getCell', sel_id, 'HOP_SUM_INSURED'),
                    ...
                    if (!$scope.saving) {
                        ...
                        s.save_policy(polhData, function (responseData) {
                            ...
                            if (responseData.state) {
                                ...
                                var bondRowIds = $scope.homeowners_grid.jqGrid("getDataIDs");
                                var covRecStatus;
                                /*---------------------------------
                                 * update motor row status
                                 *-------------------------------*/
                                for (var i = 0; i < bondRowIds.length; i++) {
                                    covRecStatus = $scope.homeowners_grid.jqGrid("getCell", bondRowIds[i], "HOP_STATUS");

                                    //Delete row if it is marked for deletion
                                    if (covRecStatus === "D") {
                                        $scope.homeowners_grid.jqGrid("delRowData", bondRowIds[i]);
                                    }
                                    //Update row status to Active (A) if it is newly added
                                    if (covRecStatus === "U") {
                                        $scope.homeowners_grid.jqGrid("setCell", bondRowIds[i], "HOP_STATUS", "A");
                                    }
                                }
        ...
        $("#btnEndorse").on('click', function () {
            u.modal_confirmation("Are you sure you want to pass an endorsement on the Policy?", function (e) {
                ...
                for (var i = 0; i < bondRowIds.length; i++) {
                    ...
                    $scope.homeowners_grid.jqGrid("setCell", bondRowIds[i], "HOP_POLH_END_NO", 22);
                }
                //Risk Covers
                ...
                if (!u.field_empty("#POLH_SYS_ID")) {
                    ...
                    $("#btnQuerypolicyreport").click(function (e) {
                        ...
                        if (u.field_empty("#POLH_ID")) {
                            ...
                            console.log(policyData);
                            var policyData = u.parse_form("#policyreportForm");
                            window.open("/PolicySchedules/PolicyHomeOwnersSchedule/" + policyData["POLH_ID"], "result", "width=900,height=1000,toolbar=0,menubar=no,status=no,resizable=yes,location=no,directories=no,scrollbars=yes");
                        }
                    });
                }
                else {
